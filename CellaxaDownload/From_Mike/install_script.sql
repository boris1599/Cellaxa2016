...
return_code := -1;
message_out := '----> Invalid currents session identifier';
...
statement := 'create sequence SE'||new_session_id;
...
if exists_out = 0 then     
return_code := -1;
message_out := '----> Invalid Current Session Identifier';
...
where session_id = curr_session_id and active_flag = 'Y';
	...
	return_code := -1;
	message_out := '----> Cannot identify user for a given current session.';
...
----------	raise NOTICE 'User level %', user_level_out;
			                ...
			                return_code := -1;
               				message_out := '----> User cannot be created. Current User is not authorized';
			               ...
			               select count(*)
               				...
               				password = encipher(password_in);
                				...
                				return_code := -1;
                				message_out := '----> User Name and/or Password exist already.';
...
return_code := -1;
message_out := '----> System ERROR. Arrays are invalid: uneven numer of elements.';  
